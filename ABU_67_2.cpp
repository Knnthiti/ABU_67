...
void ABU_Joy ::Setup_Joy_nRF24(rf24_gpio_pin_t CE, rf24_gpio_pin_t CNS, const uint64_t pipes_0, const uint64_t pipes_1) {
  ...
  openReadingPipe(true, pipes[1]);
  // setChannel(69);
}

void ABU_Joy ::Joy_Sendvalue() {
  stopListening();
  write(&Str_PS2, sizeof(Str_PS2));
}
...
void motor ::motorDrive_BTS7960(uint8_t pin_PWM, int32_t duty, uint8_t pin_LPWM, uint8_t pin_RPWM) {
  ...
  // digitalWrite(pin_LPWM, (duty = 0) ? 0 : ((duty < 0) ? 0 : 1));
  // digitalWrite(pin_RPWM, (duty = 0) ? 0 : ((duty < 0) ? 1 : 0));
...
////////////////////////////////////////////////////////class ABU_Car/////////////////////////////////////////////////////////////////
void ABU_Car ::Setup_Car_nRF24(rf24_gpio_pin_t CE, rf24_gpio_pin_t CNS, const uint64_t pipes_0, const uint64_t pipes_1) {
  ce_pin = CE;
  csn_pin = CNS;
  pipes[0] = pipes_0;
  pipes[1] = pipes_1;
...
void ABU_Car ::CLAMP(uint8_t Clamp, uint8_t UP_Clamp, uint8_t DOWN_Clamp, uint8_t Grab_Clamp, uint8_t Poll_Clamp) {
  if (attack[UP_Clamp] == 1) {
    if (digitalRead(Set_Clamp_up) == 0) {
#ifdef DEBUG_Clamp
      Serial.print("Clamp_RICE : UP-STOP ");
#endif
      digitalWrite(Clamp_up, 0);
    } else {
#ifdef DEBUG_Clamp
      Serial.print("Clamp_RICE : UP-1 ");
#endif
      digitalWrite(Clamp_up, 1);
    }
  } else {
#ifdef DEBUG_Clamp
    Serial.print("Clamp_RICE : UP-0 ");
#endif
    digitalWrite(Clamp_up, 0);
  }

  if (attack[DOWN_Clamp] == 1) {
    if (digitalRead(Set_Clamp_down) == 0) {
#ifdef DEBUG_Clamp
      Serial.print("| Clamp_RICE : DOWN-STOP ");
#endif
      digitalWrite(Clamp_down, 0);
    } else {
#ifdef DEBUG_Clamp
      Serial.print("| Clamp_RICE : DOWN-1 ");
#endif
      digitalWrite(Clamp_down, 1);
    }
  } else {
#ifdef DEBUG_Clamp
    Serial.print("| Clamp_RICE : DOWN-0 ");
#endif
    digitalWrite(Clamp_down, 0);
  }

  if (attack[Grab_Clamp] == 1) {
    if (digitalRead(Set_Clamp_grab) == 0) {
#ifdef DEBUG_Clamp
      Serial.print("| Clamp_RICE : GRAD-STOP ");
#endif
      digitalWrite(Clamp_grab, 0);
    } else {
#ifdef DEBUG_Clamp
      Serial.print("| Clamp_RICE : GRAD-1 ");
#endif
      digitalWrite(Clamp_grab, 1);
    }
  } else {
#ifdef DEBUG_Clamp
    Serial.print("| Clamp_RICE : GRAD-0 ");
#endif
    digitalWrite(Clamp_grab, 0);
  }

  if (attack[Poll_Clamp] == 1) {
    if (digitalRead(Set_Clamp_poll) == 0) {
#ifdef DEBUG_Clamp
      Serial.print("| Clamp_RICE : PULL-STOP ");
#endif
      digitalWrite(Clamp_poll, 0);
    } else {
#ifdef DEBUG_Clamp
      Serial.print("| Clamp_RICE : PULL-1 ");
#endif
      digitalWrite(Clamp_poll, 1);
    }
  } else {
#ifdef DEBUG_Clamp
    Serial.print("| Clamp_RICE : PULL-0 ");
#endif
    digitalWrite(Clamp_poll, 0);
  }
  ...
  if (attack[Clamp] == 1) {
    if (neumatic == 0) {
      neumatic = 1;  //Clamp หนีบ
      digitalWrite(Clamp_L, 1);
      digitalWrite(Clamp_R, 1);
#ifdef DEBUG_Clamp
      Serial.println("| Clamp : OPEN");
#endif
    } else {  //Clamp คาย
      neumatic = 0;
      digitalWrite(Clamp_L, 0);
      digitalWrite(Clamp_R, 0);
#ifdef DEBUG_Clamp
...
void ABU_Car ::KEEP_Ball(uint8_t button_keep, uint8_t button_UnKeep, int32_t force) {
  if (move[button_keep] == 1) {
    motorDrive_BTS7960(pin_keep, force, Keep_Ball, UnKeep_Ball);
#ifdef DEBUG_Shoot
    Serial.print("Catapult : Keep_KEEP ");
#endif

  } else if (move[button_UnKeep] == 1) {
    if (digitalRead(Set_UnKeep_Ball) == 1) {
      motorDrive_BTS7960(pin_keep, -force, Keep_Ball, UnKeep_Ball);
#ifdef DEBUG_Shoot
      Serial.print("Catapult : UnKeep_Ball ");
#endif
    } else {
      motorDrive_BTS7960(pin_keep, 0, Keep_Ball, UnKeep_Ball);
#ifdef DEBUG_Shoot
...
void ABU_Car ::Shoot_Ball(uint8_t button_Shoot, uint8_t button_UP, uint8_t button_Reload, int32_t force_Shoot, int32_t force_UP, int32_t force_Reload) {
  if (attack[button_Shoot] == 1) {
    if (digitalRead(Set_Attack_Ball) == 1) {
      motorDrive_BTS7960(pin_Shoot, force_Shoot, Attack_Ball, Reload_Ball);
#ifdef DEBUG_Shoot
      Serial.println("| Shoot_SHOOT ");
#endif
    } else {
      motorDrive_BTS7960(pin_keep, 1, Keep_Ball, UnKeep_Ball);
#ifdef DEBUG_Shoot
    ...
    if (digitalRead(Set_Attack_Ball) == 1) {
      motorDrive_BTS7960(pin_Shoot, force_UP, Attack_Ball, Reload_Ball);
#ifdef DEBUG_Shoot
    ...
    if (digitalRead(Set_Reload_Ball) == 1) {
      motorDrive_BTS7960(pin_Shoot, -force_Reload, Attack_Ball, Reload_Ball);
#ifdef DEBUG_Shoot
    ...
    motorDrive_BTS7960(pin_Shoot, 0, Attack_Ball, Reload_Ball);
#ifdef DEBUG_Shoot
    Serial.println("| Shoot_STOP ");
#endif
  }
...
void ABU_Car ::UP_speed(uint8_t button_speedUP, uint8_t button_speedDOWN) {
  if (move[button_speedUP] == 1) {
    Gear++;
  } else if (move[button_speedDOWN] == 1) {
    Gear--;
